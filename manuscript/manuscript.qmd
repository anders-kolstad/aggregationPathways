...
Although also variables may have normative properties (due to choices about how they are chosen, defined, aggregated or presented [@allain2018]) , the normalisation of variables into indicators makes indicators explicitly normative by introducing a clear directional interpretation, where increasing values always refer to an increase in condition.
...
Readers are referred to @allain2018 for a more comprehensive review of different reasons for spatially aggregating indicators.
There are also several requirements in the SEEA EA that make spatial aggregation necessary.
However, spatial aggregation also comes with a whole suite of (sometimes undesirable) side-effects, which we shall focus in now.
...
Spatially aggregating ecosystem condition metrics can be used to alleviate issues of spatial auto-correlation.
...
This approach can be seen as a type of post-stratifiction to get better variable or indicator estimates for the ecosystem assets as a whole.
It can also be motivated by a decire to know and perhaps visualise the fine-scale variation in variable value on a map.
However, this type of spatial aggregation, or perhaps better named upscaling, is not the focus of this paper (but see @nte-rel), and in our experience this has not been common practice in SEEA EA accounts produced to date.
...
In addition, averaging the indicator values across sub-regions would not produce a regional indicator value that represents its true situation as being perfectly aligned with the reference condition, since truncation of variable values above X~100~ (assuming X~100~ \> X~0~) means we are only aggregating the negative deviations from the reference level, and that very high animal densities in some sub-regions are not allowed to compensate for low values elsewhere.
...
## Some pitfalls with spatial aggregation
...
In ECAs that follow SEEA EA, information about ecosystem condition is made spatially explicit by assigning variable or indicator values to individual ecosystem assets.
...
SEEA EA encourages normalisation (which they refer to as rescaling), but other environmental assessment framework leaves more of these kinds of value judgments to the end users (ref?).
