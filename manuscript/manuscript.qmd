...
| Deer | Compensation, and discrepancy between variable and indicator (@sec-discrep) |
| PTI | Compensation and the order of operations (@sec-pathways) |
| Alien species | Agreements across scales (@sec-comp) |
...
The bilberry indicator exemplifies different spatial resolutions for the variable and for the reference levels.
...
This means one is essentially aggregating the negative deviations from the upper reference level X~100~.\
...
*Conclusion*: Given that the reference levels are the way they are, this indicator should follow pathway 3 (@fig-agg1), and the variable should be spatially aggregated from plot scale to regional scale before normalising the variable and potentially aggregating it further.
:::
...
This type of spatial aggregation, or perhaps better named upscaling, is not the focus of this paper (but see **Box 1**), and in our experience this has not been common practice in SEEA EA accounts produced to date.
...
Although variable accounts are allowed under the SEEA EA framework, ans is indeed a central feature, we believe they should be seen as interim accounts for metrics that cannot or have not yet been normalised.
...
The issue is the same as for all other variables for which normalised indicators exist, but is perhaps more extreme for WFD indicators due to different reference conditions and levels for different water body types, and strong non-linear transformation functions that inflate the aggreation displacement issuse.
<!-- Can we have a figure of calculation exmaple here? -->
:::

# Aggreements across nested ECAs {#sec-comp}

We do not presume that ECA end users will have the capacity, nor interest, to reflect very much upon the effect that spatial aggregation has had on the properties of the information they are presented with.
It should simply work, without the need for elaborate explanations.
...
For one of the variables, data sampling was random and dispersed across the EAA, and a national indicator values was calculated as some average across all sampling points.
...
In this specific case, one could in the first national account have performed a step-wise aggregation via municipalities, regions, and finally to the entire EAA, but it would not always be possible to know which ecosystem asset are to be used in future accounts.
...
Pathway 2 involves aggregating the variable before normalising[^3].
...
[^3]: In a parallel terminology, normalisation can be considered modeling, and spatial aggregation before and after normalisation can be referred to as pre or post processing, respectively [@vanbeurden1999].
...
Normalising variables early in the aggregation pathway has several benefits, such as comparability between nested assessments (Example 4, @sec-comp).
It also means that you are aggregating normative measures of condition, which is generally what you want in ECAs (@sec-norm).
