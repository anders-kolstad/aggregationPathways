...
Ecosystem accounting has been fronted as a framework for providing a knowledge base for this [@bagstad_lessons_2021; @grondard_ecosystem_2021].
Ecosystem accounts are environmental assessments aimed at communicating on the status and trends in the extent and condition of ecosystems at a scale and in a format that is relevant for decision makers [@edens2022; @comte2022].
...
However, the way spatial aggregation is done affects how a metric should be interpreted, as well as the descriptive properties and normative claims one can make based on it [@allain2018].
...
Much has been written about **vertical aggregation**, i.e. of indicators into composite **indices** [e.g. @langhans2014; @maes2020; @oecd_2008], as well as on the spatial aggregation of relatively unmodified or raw **variables** [e.g. @allain2018].
However, there is a lack of explicit deliberation on the special case of spatially aggregating highly modified **indicators** used for ecosystem condition accounting [but see @vanbeurden1999].
...
Firstly, it gives a normative interpretation of a variable, defining a good and a bad state (and possibly one or more indermediate states), and simultaneously a directionality to determine whether something is getting better or worse over time [@czucz_selection_2021].
...
Thirdly, we normalise in order to standardise the indicator on the same unitless scale, thus improving their comparability and making is possible to perform thematic (i.e. vertical) aggregation and create indices [@jakobsson2021; @pollesch_normalization_2016; @nardo_handbook_2008; @unitednations2024 §5.81].
...
Data may also be only **partly aggregated**, or the same metric may be presented at different levels of aggregation [@allain2018].
...
It is therefore common to partially aggregate the data and provide a coarser map.
...
This example is to show what it means that the variable and the reference levels need to be at a commensurable spatial scale before doing the normalisition.

*Variable*: Ground cover of European blueberry or bilberry (*Vaccinum myrtillus*) recorded in permanent vegetation plots.\
...
Given that X~100~ is defined for the extent of the aggregated area and not for each individual plot, it would make more sense for this indicator to follow pathway 3 (@tbl-pathways) by spatially aggregating the variable from plot scale to ecosystem asset to allow spatial compensation between plots.
...
Spatially exhaustive, or wall-to-wall data, include remote sensing products, but also some pre-aggregated datasets. 
For example, a lot of demographic information for example is made available at the scale of administrative units, rather than at the level of individuals (which would be very ineffective and also problematic in terms of protecting sensitive information).
Pre-aggregation may also be more subtle.
...
This issue will, for example, hamper the detection of true maximum or minimum values, since these extremes are already smoothed out in the pre-aggregation step [@vanbeurden1999].
...
Spatial prediction can be used to assign variable or indicator values to all areas (as is required in the SEEA EA framework).
...
For example, if using interpolation to create local predictions for a variable across an ecosystem asset, then a simple systematic sampling of the field of values across the ecosystem asset will be valid, and give representative grounds for statistical inference [@brus2021].
...
In addition, non-parametric bootstrapping [@kolstad_area_2025] and Bayesian updating [@kolstad_making_2024] can be used to possibly obtain better descriptions of the uncertainty in the estimates.
...
However, this decision clearly affects the results of the spatial aggregation via a well-described (but often forgotten?) issue referred to as the **modifiable area unit problem (MAUP)**.
...
Spatial aggregation leads to a divergence in the type of information held in variables and in their associated indicators, to a point where they become qualitatively very different (**Examples 2 and 3**).
...
In the statistical standard and recommended guidelines for ecosystem accounts [@unitednations2024; -@unitednations2022], there is no mention of aggregation displacement, and there seems to be little awareness in general of its meaning and the ramification of choosing different spatial aggregation pathways for variables.
...
This practice in unproblematic in the (rare) case where an indicator is obtained by a strictly linear normalisation of the variable, but it cannot be recommended when the normalisation involves truncation (**Example 2**) and/or transformation (**Example 3**).
...
More specifically, it shows how spatial compensation in the spatial aggregation of variables leads to aggregation displacement and a discrepancy between the variable and the indicator.
...
This number is informed by data on habitat quality, food supply, and more.
X~0~ = no wolves.
...
Because variable values above X~100~ are truncated, the high wolf numbers in EA 1 does not compensate for the low indicator value in assets two and three, and hence the mean indicator value is less than the sum of the variable values divided by the sum of the X~100~values.
...
A major determinant of the resulting aggregation displacement from the spatial aggregation of ecosystem condition indicators (@fig-agg1) is the choice about the order of the different steps in the normalisation and aggregation process, i.e. the aggregation pathway (@tbl-pathways).
...
The Water Framework Directive (WFD) indicators use aggregation pathway 5 (@tbl-pathways), 
...
Instead, the aggregated reporting metric is the proportion of water bodies in each of the five condition categories 
...
In @tbl-pathways we show some aggregation pathways that we have come across, and some that we see as potential new pathways.
...
Instead one reports the proportion of water bodies in each condition class (spatial statistics).
Pathway 6 is a suggested pathway for handling WFD indicator in ECAs (see **Example 3**), where truncation is postponed until after the spatial aggregation in order to allow overshooting values to compensate for low indicator values elsewhere (what we refer to as compensation), and  in order to correctly handle indicators that are calculated using non-linear transformation functions..
...
Pathways 8 and 9 describe alternative ways to create indices at different spatial scales, either by performing the spatial aggregation on the indices (i.e. after thematic aggregation; pathway 8) or on the indicators (i.e. before thematic aggregation; pathway 9).
If all indicators making up an index are spatially aggregated using the same weights, typically the extent of the spatial units, then the aggregation operations will be commutative and it will not matter if you aggregate vertically then horizontally, or the other way around (@tbl-pathways; Pathway 8 and 9, respectively) [@unitednations2024].
However, indicators are not always valid for the entire ecosystem type that the account is made over.
They can, for example, retain to only certain habitats or nature types within the ecosystem type, in which case one might want to use different weights when spatially aggregating the indicator that reflect the extent of only the relevant habitat or nature types, and not of the entire ecosystem.
This can be achieved using pathway 9. 



```{r tbl-pathways}
#| tbl-cap: |
#|   Table showing  a selection of nine possible pathways for the spatial aggregation of variables in ecosystem condition accounting. Normalisation refers to the steps that make a variable conform to the definition of an indicator in SEEA EA, which may include some, but not necessarily all, of the steps scale, truncate and transform. The pathway description uses the standardised shorthand aggregation pathway terminology, where trunc. = truncation, trans. = transformation, sp. agg. = spatial aggregation, proj. = project (extrapolation or interpolation), sample = sub-sampling from a larger dataset, th. agg. = thematic aggregation.
#| out-width: 600
#| echo: false
...
We suggest using the terms as described in @fig-term, @tbl-terms and elsewhere in the paper: scale, truncate (abbreviated to *trunc.*), transform (*trans.*), spatial aggregation (*sp. agg.*), thematic aggregation (*th. agg.*) and spatial statistics (*sp. stat.*).
...
For example, pathway 1 in @tbl-pathways can be annotated as $scale - trunc. - trans. - sp. agg.$
...
The method for spatial aggregation should be included in parentheses in a similar manner, i.e. $sp. agg. (area-weighted arithmetic mean)$. 
Note also that in the last example, normalisation only involved a scaling step.
The description of indicator pathways can be made even easier by referencing specific aggregation pathways by number, as we have done in this paper using @tbl-pathways in this paper.
...
Thus, pathway 7 in @tbl-pathways can be written as $proj. - norm. - sample - sp.agg.$.
The type of model used for the spatial projection can be added as a footnote.

Besides making the indicator workflow itself more transparent and reproducible, we believe this recommendation would help raise the general awareness about aggregation displacement in ecosystem accounting, both among users and developers.
...
Normalising variables early in the aggregation pathway means that you are aggregating normative measures of condition, which is generally what you want in ECAs (@sec-norm, **Example 3**).
...
### 5. Do not aggregate indices, unless you know all aggregation operations are commutative {.unnumbered}

When spatially aggregating indices (@tbl-pathways; pathway 8) you loose the ability to give different weights to different indicators.
The result can be that the aggregated index value will not be determinable from the aggregated indices, 
and that  ECAs using the same indicator set, 
but focusing on different spatial scales 
(e.g. one one presneting regional values only, and anther also presenting local values), 
will not get equal index values for the same areas, even though they use the same indicator set. We therefore recommend caution, and to default to the solution of instead of spatially aggregating indicators separately, and then re-calculating indices when needed (@tbl-pathways; Pathway 9).
...
### 6. Do not present variables and their respective indicators as alternative evidence {.unnumbered}

Aggregation displacement causes a divergence in the type of information held in ecosystem condition variables versus ecosystem condition indicators, and spatially aggregated indicators should not be interpreted as normalised versions of aggregated variables.
Therefore, aggregated variables and the associated aggregated indicators cannot act as alternative evidence or interpretations for the ecosystem condition.
...
Misunderstandings and misuse can ensue, for example by taking the variable value and assigning it the same interpretation as the indicator value.
...
From the above, we can derive three sub-recommendations: (a) Never mix indicators and variables in the same account or in the same table; (b) Only report ecosystem condition variables if the corresponding indicators are unavailable; (c) Always prefer to report ecosystem condition indicators if they are available. 
...
We have showed that identical raw data, when spatially aggregated through different pathways, can yield fundamentally different types of information about ecosystem condition. 
The recommendations offered here are intended to help researchers 
...
With a clearer understanding of the challenges inherent to spatial generalisation,
we hope that future ecosystem condition accounts will be more internally consistent, more comparable among each other, have a higher level of reproducibility, and be less susceptible to unintended mathematical artefacts.   

# CRediT statement

**Anders Kolstad**: Conceptualization, Data Curation, Writing - Original Draft, Visualization, Funding acquisition.
**Hanno Sandvik**: Conceptualization, Writing - Review & Editing, Visualization.
**Bálint Czúcz**: Conceptualization, Writing - Review & Editing.
**Chloé R. Nater**: Conceptualization, Writing - Review & Editing.

# Funding

This work received partial funding by the Norwegian Environment Agency (nr. 22047007).
