...
The ecological significance of the indicator might also differ from that of the variable since additional ecological knowledge can be introduced via the reference leveles.
...
In the Norwegian Natur Index [@jakobsson2020], the variable follows pathway 1 and is normalised at the CMA level.
...
In this example, The Norwegian Nature Index uses the same pathway for all indicators, so this common missunderstanding about what indicator values represenets can be exaplin just one time, and not for each indicator spesifically.
...
### 1. Always report the aggregation pathway, using standardised terminology {.unnumbered}

In indicator documentation or ECA repports, authors should make an effort to make a precise description of the steps in the aggregation process.
...
Besides making the indicator workflow itself more transparrent and reproduciable, ee believe this recommendation would help raise the general awareness about aggregation bias in EAs, both among readers and researchers.
...
### 2. Always normalise variables at the scale where the reference levels are relevant {.unnumbered}

Normalising variables early in the aggregation pathway has several benefits, as we have seen.
...
This was exeplified with the billberry example (@sec-bill).

### 3. Use the same aggregation pathway for all indicators in the same assessment {.unnumbered}

As we saw in the wolves example (@sec-wolves) this eases interpretation and reduces the change that the meaning of indicators are misunderstood.
It may be premature, or not even possible, to prescribe an aggregation pathway to be used for all ECA, but internal consistency within assessment should be possible.
...
Yet different versions are often refered to by the same common name.
