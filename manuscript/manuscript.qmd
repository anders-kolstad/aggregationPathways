...
This smoothing out makes the data and its inherent information less suitable for other uses, such as depicting spatial variation that can be used to support local land use planning, or for aiding a debate about spatial conflicts.
...
In conclusion, there are several requirements in the SEEA EA that make spatial aggregation necessary, but spatial aggregation also comes with a whole suite of (sometimes undesirable) side-effects.
...
However, we are here mostly interested in what we consider an extended case of aggregation error, sensu @rastetter1992. They showed that when we take the average of two variables, each of which have associated attribute values (e.g. indicator values), then the attribute of the average variable value differs from the average of the attributes of the original variables.
...
Also, in ecosystem condition accounting (ECA) specifically, the aggregation of both variables and indicators is common practice and raises the issue of **aggregation bias** (@fig-agg1; @fig-agg2) when variables have nonlinear relationships to the normalised indicators [@rastetter1992], which they typically do.
