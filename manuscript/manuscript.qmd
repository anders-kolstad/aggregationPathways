...
One can assign values to areas via an aggregation function used on data values that fall within a given ecosystem asset.
...
This approach can also be seen as a type of post-stratification, and it can result in better variable or indicator estimates for the ecosystem assets as a whole.
...
The term spatial aggregation is also sometimes used to refer to **upscaling** in the sense of taking local information and making it applicable or translating it to larger, e.g. landscape or regional, scales [@knotters2010; @finke2002].
...
Further still, data may not be available at the original spatial scale, but may only be downloadable in pre aggregated form [@vanbeurden1999].
...
However, in our case the difference between variables and indicators is an intentional one, and we therefore refer to what is essentially the same mathematical effect, with the less derogatory term aggregation displacement.
...
From this, many different conclusions can be made, depending on what the user considers high or low densities.
...
# Aggregements across nested ECAs {#sec-comp}
...
The examples are non-exhaustive, and serve mainly to illustrate that many different pathways can be created using a limited number of unique operations (@sec-rec).
...
Pathway 3 is similar to 2, but here there are two aggregation steps: one before and one after normalisation.
...
Yet this would in many cases be needed if the individual indicators do not all represent the entire ecosystem inside each ecosystem asset.
