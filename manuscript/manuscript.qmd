...
Ecosystem accounting (EA) has been fronted as a framework for providing a knowledge base for this.
...
In 2026, EU and EEA countries will be required to report ecosystem condition accounts to Eurostat and the European Commission (?), and these accounts will have to be spatially aggregated to national levels.
...
Intertwinement can be explicit, for example when using model-based spatial estimation methods (e.g. geostatistics) and subsequent spatial sampling of this field of random values [@brus1997].
In other cases, inter- or extrapolation can also be combined with spatial aggregation more inadvertently, for example by averaging point data and attributing this value to entire polygons, thus relying on a design-based estimation process [@gruijter1990].
...
The ecological fallacy [@robinson2009] is the error of assuming that relationships or patterns observed for groups (at the aggregate level) must also hold true for the individuals within those groups.
In EA, aggregated data will mask differences within regions and lead some people to interpret this as regions being more homogenous then they really are.
:::
...
Methods used to change either the cover or the extent of the indicator data (see @sec-term) can also be included in the shorthand notation scheme.
