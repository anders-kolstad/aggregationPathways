...
Without an understanding of the pitfalls during spatial aggregation of information, one risks introducing unintended and undetected systematic bias, as well as downright logical errors.
...
This paper therefore focus on explaining and providing guidelines on the spatial aggregation inherent to the System of Environmental Economic Accounting - Ecosystem Accounting (SEEA EA) [@unitednations2024] - the most used ecosystem accounting (natural resource accounting) framework in the world [@comte2022].
...
SEEA EA distinguishes between indicators and variables, and to be able to describe how these are affected by spatial aggregation, we need to spend some time to explaining the difference between them.
...
Sometimes indicators are calculated directly, but usually indicators are made by **normalising** variables based on two or more **reference levels** [@unitednations2024 §5.60]. Although also variables may have normative properties (due to choices about how they are chosen, defined, aggregated or presented [@allain2018]) , the normalisation of variables into indicators makes indicators explicitly normative by introducing a clear directional interpretation, where increasing values always refer to an increase in condition.
...
The process of assigning values to ecosystem assets requires some spatial manipulations of data, or scale changes [@ewert2011].
...
<!--# Subsequently, spatial aggregation is required when going from ecosystem assets to large spaatial scale, ultimately the scale of the entire ecosystem accounting area (EAA).  -->

Complete, or wall-to-wall data, include remote sensing products, as well as other types of pre-aggregated data, where sparse data are assigned to areal units.
...
Also data such as rasters from satellite images ,or maps derived from such, have some inherent pre-aggregation where values are averaged for each raster cell.
...
This deliberate spatial aggregation is the main focus of this paper.
When doing this aggregation, we need to account for the spatial structure (spatial correlations) of the data (@sec-auto).
...
There may be limitations with the data, related to the type of pre-aggregation performed (see above).
In other cases the areal units are decided politically, with little scientific reasoning behind it.
...
Interpolation or other modeling can also be used to impute or predict data values between the points where we have information [@finke2002; @stein2001].
...
| Term | Definition |
|----|----|
| Metric | A collective term for both variables and indicators |
| Reference levels | Variable values corresponding and anchoring the variable to specific indicator values. A special terminology is used, based on @czúcz, which differs somewhat from that in @unitednations2024. See @fig-term. |
| Scaling | A linear shrinking or enlarging the variable data range by anchoring X~0~ and X~100~ to the values 0 and 1, respectively. See @fig-term. |
| Transformation |  |
| Truncation |  |
| Normalisation | Referred to as rescaling in @unitednations2024. This is the process of converting a variable to an indicator. |
| Compensation |  |
| Aggregation pathway |  |
