...
This represents a type of knowledge synthesis, aimed at condensing the complexity of the real world into something that is relevant to decision makers and perhaps actionable at a political level [@king2024].
...
And finally, in @sec-rec we give five concrete recommendations aimed at indicator developers that will help ensure a sensible and informed approach to spatial scaling of ecosystem condition metrics.
...
However, **Example 4** demonstrates two factors that lead to this indicator-level commutativity is not being fulfilled in this case.

First, some indicators are possible normalise at the measurement scale, i.e. the finest resolution of the raw data.
But others may need to be (pre-) aggregated, for example if the variable is designed to depict a density or a proportion that retains to a spatial unit.
...
It turn out that in this case the indicator value at the EAA level is not insensitive to the spatial delineation of the EAs.
...
*Reference levels*: X~100~ is defined as the extent of glacier area *anno* 1947-1985.
...
The point estimates for the variable and the reference levels for each EA was then turned into probability distributions by sampling from a normal distribution with a standard deviation equal to 3% of the mean (the uncertainty reported for the maps).
...
It also meant that areas inside the EA with no initial glacier cover was seen as similarly non-relevant to the indicator.
...
If all indicators making up n index are spatially aggregated using the same weights, typically the extent of the spatial units, then the aggregation operations will be commutative and it will not matter if you aggregate vertically then horisontally , or the other way around (\@fig-pathways; Pathway 8 and 9, respectively).
...
This will ensure that nested accounts will be comparable, i.e. that they will present the same index values independent of which spatial units that the indices first are calculated for.

An alternative solution to ensure agreement across nested accounts is to use weights also when performing the vertical or thematic aggregation of indicators into indicators, where indicators are weighted respective of the area that they are valid over.
With this approach one can down-weigh indicators that only cover only smaller areas (specific nature types etc.), or which have incomplete spatial coverage.
When this weighting is already performed at the vertical aggregation stage, any subsequent horisontal aggregation does not need to account for the unique indicator weights again.
