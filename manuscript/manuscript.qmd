...
Using models to predict variable or indicator values is allowed under the SEEA EA. However, in this paper we are addressing the second part of this process only, of simplifying the spatial variation, from many to few data points.

##### Modifiable area unit problem {.unnumbered}

The modifiable area unit problem (MAUP; [@openshaw1984, @wong2009]) describes the sensitivity that spatially aggregated information has on the choice about the spatial units used for either collecting the data or for aggregating the data to. These units can be arbitrary (*modifiable*), yet different sizes and configuration of units can have large effects on the outcome of aggregation. Administrative units are often used as the ecosystem assets in SEEA EAs, yet they are in an ecological sense almost always arbitrary, and can also be subject to change over time. The original spatial units that data are collected at may be a pragmatic choice in much the same way. Further still, data may not be available at the original spatial scale, but may only be downloadable in preaggregated form [@vanbeurden1999].
...
In ECAs, data are often not wall-to-wall, and when upscaling is not done, as defined above, the spatial aggregation needs to account for the spatial structure of the data in a different way, such as design-based approached (relying on random or balanced samples across the entire EEA), or a step-wise aggregation via ecosystem assets, homogenous ecosystem areas [@vallecillo2022] or homogenous impact areas [@kolstad_making_2024].
...
When using means, area weighting is required when following SEEA EA.
...
# Compliance across nested assessments
